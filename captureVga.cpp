...
void CaptureVGA::savetofile(char * ptr) {
	int fd;

	fd = open("dump_yuv.yuv", O_CREAT | O_WRONLY | O_APPEND);

	if (fd < 0)
		perror("open");

	char* pTemp = new char[capt.fmt.fmt.pix.width * capt.fmt.fmt.pix.height * 2];
	ColorConvertUtils::nv12_to_yuv420((unsigned char*)pTemp,
...
bool CaptureVGA::setupCapture() {
	...
	if (ret < 0) {
		printf("Get Current Format failed\n");
		return false;
	}
	printf("step 1\n");
...
bool CaptureVGA::queueCaptureBuffers() {
	...
	for (i = 0; i < MAX_BUFFER; i++) {
		...
		if (ret < 0) {
			printf("queue capture buffers VIDIOC_QBUF,%d\n",&ret);
			return false;
		}
		printf("queue capture buffers VIDIOC_QBUF\n");
	}
	return true;
}
...
bool CaptureVGA::onCaptureTask() {
	printf("onCaptureTask\n");
	int i = 0, ret = 0;
	//while (1) {
	//	if (m_bExit) {
	//		break;
	//	}
	    printf("onCaptureTaskLoop\n");

		ret = ioctl(capt.fd, VIDIOC_DQBUF, &capt.buf);
		if (ret < 0) {
			//perror("on capture task VIDIOC_DQBUF\n");
			printf("capture fd is %d, ret %d %d\n", capt.fd, ret,errno);
//			continue;
		}
...
//			continue;
		...
		printf("on capture task VIDIOC_QBUF successed\n");
//	}
...
bool CaptureVGA::start() {
	...
	printf("Create vga capture Task successed !\n");
	m_bInited = true;
	return true;
}
